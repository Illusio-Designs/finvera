...
export default function TDSPaymentPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Create ledger entries for TDS payment
      // Debit TDS payable ledger, Credit Bank/Cash
      const ledgerEntries = [
        {
          ledger_id: formData.tds_ledger_id,
          debit_amount: amount,
          credit_amount: 0,
          narration: `TDS Payment - Section ${formData.tds_section || 'N/A'}`,
        },
        {
          ledger_id: formData.bank_ledger_id,
          debit_amount: 0,
          credit_amount: amount,
          narration: 'TDS Payment to Government',
        },
      ];

      await accountingAPI.vouchers.create({
        voucher_type: 'TDS Payment',
        voucher_date: formData.voucher_date,
        narration: formData.narration || `TDS Payment - Section ${formData.tds_section || 'N/A'} - Challan: ${formData.challan_number || 'N/A'}`,
      ...
      toast.success('TDS payment voucher created successfully');
      router.push('/client/vouchers/vouchers');
    } catch (error) {
      const errorMessage = error.response?.data?.message || error.response?.data?.error || 'Failed to create TDS payment voucher';
      toast.error(errorMessage);
      console.error('Error creating TDS payment voucher:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <ProtectedRoute portalType="client">
      <ClientLayout title="TDS Payment">
        <PageLayout
          title="Create TDS Payment Voucher"
          breadcrumbs={[
        ...
        >
          <form onSubmit={handleSubmit} className="space-y-6">
            <Card>
              <div className="mb-4">
                <h2 className="text-lg font-semibold text-gray-900">TDS Payment Details</h2>
                <p className="text-sm text-gray-500 mt-1">Record payment of TDS to government</p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <FormInput
                  ...
                  placeholder="Enter TDS challan number"
                />
