...
export default function InventoryPage() {
  ...
  const handleEdit = async (id) => {
    try {
      ...
      // Load warehouse stock for this item
      try {
        const stockResponse = await accountingAPI.inventory.items.getWarehouseStock(id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Save warehouse opening stocks (only for new items, or update existing)
              ...
              <div className="mb-4">
                ...
                <p className="text-sm text-gray-600 mt-1">
                  {editingId
                    ...
                    : 'Add a new inventory item to track stock and manage inventory'}
                </p>
              </div>
                ...
                <div className="border-t pt-6">
                  ...
                  {warehouseOpeningStocks.length > 0 && (
                    <div className="space-y-4">
                      {warehouseOpeningStocks.map((ws, index) => (
                        <div
                          key={index}
                          className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 bg-gray-50 rounded-lg border border-gray-200"
                        >
                          <FormSelect
                            ...
                            onChange={(name, value) =>
                              updateWarehouseOpeningStock(index, 'warehouse_id', value)
                            }
                          ...
                          <FormInput
                            ...
                            value={ws.quantity}
                            onChange={(name, value) =>
                              updateWarehouseOpeningStock(index, 'quantity', value)
                            }
                            placeholder="0"
                            required
                          />
                          <FormInput
                            ...
                            value={ws.avg_cost}
                            onChange={(name, value) =>
                              updateWarehouseOpeningStock(index, 'avg_cost', value)
                            }
                            placeholder="0.00"
                            required
                          />
                  ...
                  {warehouseOpeningStocks.length === 0 && (
                    <div className="text-center py-8 text-gray-500">
                      <p>
                        No warehouse opening stock entries. Click &quot;Add Warehouse&quot; to add one.
                      </p>
                    </div>
                  )}
                </div>
