...
export default function GSTR1() {
  ...
  const { data, loading, execute } = useApi(
    () => gstAPI.returns.generateGSTR1({ period, gstin_id: gstinId }),
    false
  );
  ...
  const handleDownload = () => {
    if (data?.data?.json_file) {
      ...
      link.download = `GSTR1_${period}.json`;
      link.click();
    } else {
  ...
  return (
    <ProtectedRoute>
      <ClientLayout title="GSTR-1 - Client Portal">
        <Toaster />
        <PageLayout
              ...
              <dl className="space-y-4">
                ...
                {data.data.total_invoices && (
                  <div>
                    ...
                    <dd className="mt-1 text-sm text-gray-900">{data.data.total_invoices}</dd>
                  </div>
                )}
              </dl>
            </Card>
