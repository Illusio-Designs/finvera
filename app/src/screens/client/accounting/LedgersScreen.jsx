...
export default function LedgersScreen() {
  ...
  const fetchLedgers = useCallback(async () => {
    try {
      ...
      showNotification('Failed to load ledgers', 'error');
      setLedgers([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleLedgerPress = (ledger) => {
    showNotification(`View details for ${ledger.name}`, 'info');
  };

  const handleCreateLedger = () => {
    showNotification('Create new ledger functionality will be available soon', 'info');
  };
