...
export default function SalesInvoiceScreen() {
  ...
  const fetchVouchers = useCallback(async () => {
    ...
    try {
      ...
      const response = await voucherAPI.salesInvoice.list(params);
      const data = response?.data?.data || response?.data || [];
      setVouchers(Array.isArray(data) ? data : []);
    } catch (error) {
      console.error('Sales vouchers fetch error:', error);
      showNotification({
        type: 'error',
        title: 'Error',
  ...
  const handleVoucherPress = async (voucher) => {
    try {
      ...
      const response = await voucherAPI.salesInvoice.get(voucher.id);
        ...
        {loading ? (
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptySubtitle}>
              Click "New Invoice" button above to create your first sales invoice
            </Text>
          </View>
        ) : (
