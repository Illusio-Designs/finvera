...
export default function SalesInvoiceScreen({ navigation, route }) {
  ...
  const loadInvoiceData = async () => {
    try {
      const response = await voucherAPI.salesInvoice.get(voucherId);
  ...
  const generateInvoiceNumber = () => {
    ...
    const invoiceNumber = `SI-${year}${month}-${random}`;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEdit) {
        await voucherAPI.salesInvoice.update(voucherId, invoiceData);
      } else {
        await voucherAPI.salesInvoice.create(invoiceData);
      }

      Alert.alert(
        'Success',
        `Sales invoice ${isEdit ? 'updated' : 'created'} successfully`,
        [
        ...
        <View style={styles.section}>
          ...
          <View style={styles.inputContainer}>
            ...
            <TextInput
              ...
              onChangeText={(value) => handleInputChange('customerGstin', value.toUpperCase())}
