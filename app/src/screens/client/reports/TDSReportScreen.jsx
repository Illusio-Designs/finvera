...
export default function TDSReportScreen() {
  ...
  const generateReport = async () => {
    ...
    try {
      setLoading(true);
      const response = await taxAPI.tds.report(filters);
      ...
      showNotification({
        type: 'error',
        title: 'Error',
        message: error.response?.data?.message || 'Failed to generate TDS report'
      });
    } finally {
      setLoading(false);
    }
  ...
  const tdsSection = [
    '194Q', '194C', '194J', '194I', '194H', '194A', '194D', '194G', '194LA', '194LBA'
  ];
      ...
      >
        ...
        <View style={styles.headerSection}>
          <View style={styles.headerIcon}>
            <Ionicons name="document-text" size={32} color="#3e60ab" />
          </View>
          <Text style={styles.headerTitle}>TDS Report</Text>
          <Text style={styles.headerSubtitle}>
            Tax Deducted at Source summary and details
          </Text>
        </View>
        ...
        <View style={styles.filterCard}>
          ...
          <View style={styles.formGroup}>
            <Text style={styles.label}>TDS Section (Optional)</Text>
          ...
          <View style={styles.formGroup}>
            <Text style={styles.label}>Deductee PAN (Optional)</Text>
            <TextInput
              style={styles.input}
              value={filters.deductee_pan}
              onChangeText={(text) => setFilters(prev => ({ ...prev, deductee_pan: text.toUpperCase() }))}
            ...
            <View style={styles.summarySection}>
              ...
              <View style={styles.summaryGrid}>
                ...
                <View style={styles.summaryCard}>
                  ...
                  <Text style={styles.summaryValue}>
                    {formatCurrency(reportData.summary?.total_tds_amount || 0)}
                  </Text>
                ...
                <View style={styles.summaryCard}>
                  ...
                  <Text style={styles.summaryValue}>
                    {formatCurrency(reportData.summary?.total_payment_amount || 0)}
                  </Text>
                  <Text style={styles.summaryLabel}>Total Payment Amount</Text>
                </View>
              </View>
            </View>
                ...
                {reportData.transactions.map((txn, index) => (
                  <View key={index} style={styles.transactionCard}>
                    <View style={styles.transactionHeader}>
                      <Text style={styles.transactionDeductee}>{txn.deductee_name}</Text>
                      <Text style={styles.transactionAmount}>{formatCurrency(txn.tds_amount)}</Text>
                    </View>
                    <View style={styles.transactionDetails}>
                      <View style={styles.transactionDetail}>
                        <Ionicons name="person-outline" size={14} color="#64748b" />
                        <Text style={styles.transactionDetailText}>PAN: {txn.deductee_pan}</Text>
                      </View>
                      <View style={styles.transactionDetail}>
                        <Ionicons name="document-text-outline" size={14} color="#64748b" />
                        <Text style={styles.transactionDetailText}>Section: {txn.tds_section}</Text>
                      </View>
                      <View style={styles.transactionDetail}>
                        <Ionicons name="calendar-outline" size={14} color="#64748b" />
                        <Text style={styles.transactionDetailText}>
                          Date: {new Date(txn.deduction_date).toLocaleDateString('en-IN')}
                        </Text>
                      </View>
                    </View>
                  </View>
        ...
        {!reportData && !loading && (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyStateText}>
              Select filters and click "Generate Report" to view TDS summary
            </Text>
          </View>
        )}
      </ScrollView>
