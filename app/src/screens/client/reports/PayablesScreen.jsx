...
export default function PayablesScreen() {
  ...
  const fetchPayables = useCallback(async () => {
    ...
    try {
      const response = await reportsAPI.payables({ 
        as_on_date: asOnDate,
      ...
      showNotification({
        type: 'error',
        title: 'Error',
        message: 'Failed to load payables report'
      });
  ...
  const generatePDF = async () => {
    if (!payablesData) {
      showNotification({
        type: 'error',
        title: 'No Data',
        message: 'No payables data available to export'
      });
      return;
    }

    try {
      const htmlContent = generatePayablesHTML();
      ...
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(uri, {
          ...
          dialogTitle: 'Share Payables Report',
          UTI: 'com.adobe.pdf',
        });
      } else {
        showNotification({
  ...
  const generatePayablesHTML = () => {
    const { payables, summary, as_on_date } = payablesData;
    ...
    return `
      ...
      <body>
        <div class="header">
          ...
          <div class="report-title">Payables Report (Sundry Creditors)</div>
          <div style="font-size: 12px;">As on ${formatDateToDDMMYY(as_on_date)}</div>
        </div>
        
        <div class="summary-box">
          <div class="summary-item">
            <span><strong>Total Suppliers:</strong></span>
            <span>${summary?.total_suppliers || 0}</span>
          </div>
          <div class="summary-item">
            <span><strong>Total Payable:</strong></span>
            <span>${formatCurrency(summary?.total_payable || 0)}</span>
          </div>
          <div class="summary-item">
            <span><strong>Average per Supplier:</strong></span>
            <span>${formatCurrency(summary?.average_payable || 0)}</span>
          </div>
        </div>
        ...
        <table class="tally-table">
          <thead>
            <tr>
              ...
              <th style="width: 40%;">Amount Payable</th>
            </tr>
          </thead>
          <tbody>
            ${generatePayablesRows(payables)}
            <tr class="total-row">
              <td><strong>Total</strong></td>
              <td class="amount"><strong>${formatCurrency(summary?.total_payable || 0)}</strong></td>
            </tr>
          </tbody>
        </table>
  ...
  const generatePayablesRows = (payables) => {
    if (!payables || payables.length === 0) return '<tr><td colspan="2" style="text-align: center;">No payables found</td></tr>';
        ...
        {loading ? (
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptySubtitle}>
              No payables data available for the selected date
            </Text>
          </View>
        ) : (
          <>
            ...
            <View style={styles.tableContainer}>
              <Text style={styles.tableTitle}>Supplier Balances</Text>
              <ScrollView 
                horizontal={true}
                ...
                <View style={styles.table}>
                  <View style={styles.tableHeader}>
                    ...
                    <View style={styles.tableHeaderAmount}>
                      <Text style={styles.tableHeaderText}>Amount Payable</Text>
                    </View>
                  </View>
                  ...
                  >
                    <View style={styles.tableBody}>
                      {payablesData.payables && payablesData.payables.length > 0 ? (
                        payablesData.payables.map((item, index) => (
                        ...
                        <View style={styles.emptyRow}>
                          <Text style={styles.emptyRowText}>No payables found</Text>
                        </View>
                      )}
                ...
                <View style={styles.agingCards}>
                  <View style={styles.agingCard}>
                    ...
                    <Text style={styles.agingValue}>
                      {formatCurrency(payablesData.aging_analysis.current?.amount || 0)}
                    </Text>
                    <Text style={styles.agingCount}>
                      {payablesData.aging_analysis.current?.count || 0} suppliers
                    </Text>
                  </View>
                  <View style={styles.agingCard}>
                    <Text style={styles.agingLabel}>31-60 days</Text>
                    <Text style={[styles.agingValue, { color: '#f59e0b' }]}>
                      {formatCurrency(payablesData.aging_analysis.days_31_60?.amount || 0)}
                    </Text>
                    <Text style={styles.agingCount}>
                      {payablesData.aging_analysis.days_31_60?.count || 0} suppliers
                    </Text>
                  </View>
                  <View style={styles.agingCard}>
                    <Text style={styles.agingLabel}>61-90 days</Text>
                    <Text style={[styles.agingValue, { color: '#f97316' }]}>
                      {formatCurrency(payablesData.aging_analysis.days_61_90?.amount || 0)}
                    </Text>
                    <Text style={styles.agingCount}>
                      {payablesData.aging_analysis.days_61_90?.count || 0} suppliers
                    </Text>
                  </View>
                  <View style={styles.agingCard}>
                    <Text style={styles.agingLabel}>Over 90 days</Text>
                    <Text style={[styles.agingValue, { color: '#dc2626' }]}>
                      {formatCurrency(payablesData.aging_analysis.over_90?.amount || 0)}
                    </Text>
                    <Text style={styles.agingCount}>
                      {payablesData.aging_analysis.over_90?.count || 0} suppliers
                    </Text>
                  </View>
                </View>
              </View>
