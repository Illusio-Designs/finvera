...
export default function CreateCreditNoteModal({ 
  ...
  const fetchCustomers = async () => {
    try {
      ...
      setCustomers(Array.isArray(sundryDebtors) ? sundryDebtors : []);
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      showNotification({
        type: 'error',
        title: 'Error',
        message: error.response?.data?.message || 'Failed to create credit note'
      });
    } finally {
      setLoading(false);
    }
  ...
  const selectedCustomer = customers.find(c => c.id === formData.party_ledger_id);
    ...
    >
      <View style={styles.container}>
        <View style={styles.header}>
          <Text style={styles.headerTitle}>Create Credit Note</Text>
        ...
        <ScrollView style={styles.content}>
          <View style={styles.section}>
            ...
            <View style={styles.field}>
              ...
              >
                <Text style={[styles.selectButtonText, selectedCustomer && styles.selectButtonTextSelected]}>
                  {selectedCustomer ? selectedCustomer.ledger_name : 'Select Customer'}
                </Text>
              ...
              {loadingCustomers ? (
                ...
                customers.map((customer) => (
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      setFormData({ ...formData, party_ledger_id: customer.id });
                      setShowCustomerModal(false);
                    }}
                  >
                    <Text style={styles.listItemText}>{customer.ledger_name}</Text>
                    {formData.party_ledger_id === customer.id && (
