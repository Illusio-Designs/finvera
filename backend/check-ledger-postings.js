...
async function checkLedgerPostings() {
  try {
    ...
    // Step 2: Check all ledger entries for this voucher with account groups
    console.log('\n2️⃣ VOUCHER LEDGER ENTRIES');
    console.log('-'.repeat(40));
    
    // First get ledger entries from tenant DB
    ...
    // Get account group names from master DB
    const accountGroupIds = [...new Set(ledgerEntries.map(entry => entry.account_group_id).filter(id => id))];
    let accountGroups = {};
    
    if (accountGroupIds.length > 0) {
      const groupIdsList = accountGroupIds.map(id => `'${id}'`).join(',');
      const [groups] = await masterSequelize.query(`
        SELECT id, name, group_code FROM account_groups WHERE id IN (${groupIdsList})
      `);
    ...
    // Step 3: Check current balances of affected ledgers
    console.log('\n3️⃣ CURRENT LEDGER BALANCES');
    console.log('-'.repeat(40));
    
    const ledgerIds = ledgerEntries.map(entry => `'${entry.ledger_id}'`).join(',');
    
    const [ledgerBalances] = await tenantSequelize.query(`
      SELECT 
        l.id,
        l.ledger_name,
        l.ledger_code,
        ...
        (COALESCE(SUM(vle.debit_amount), 0) - COALESCE(SUM(vle.credit_amount), 0)) as calculated_balance
      FROM ledgers l
      LEFT JOIN voucher_ledger_entries vle ON l.id = vle.ledger_id
      WHERE l.id IN (${ledgerIds})
      GROUP BY l.id, l.ledger_name, l.ledger_code, l.current_balance, l.balance_type, l.account_group_id
      ORDER BY l.ledger_name
    `);
    
    console.log(`Checking balances for ${ledgerBalances.length} affected ledgers:`);
    
    ledgerBalances.forEach((ledger, index) => {
      const currentBalance = parseFloat(ledger.current_balance || 0);
      const calculatedBalance = parseFloat(ledger.calculated_balance || 0);
      const totalDebits = parseFloat(ledger.total_debits || 0);
      const totalCredits = parseFloat(ledger.total_credits || 0);
      
      console.log(`\n  Ledger ${index + 1}: ${ledger.ledger_name}`);
      console.log(`    Code: ${ledger.ledger_code || 'N/A'}`);
      console.log(`    Group: ${ledger.account_group_name || 'N/A'}`);
      console.log(`    Balance Type: ${ledger.balance_type || 'N/A'}`);
    ...
    const [gstLedgers] = await tenantSequelize.query(`
      ...
      GROUP BY l.id, l.ledger_name, l.ledger_code, l.current_balance, ag.name
      ORDER BY l.ledger_name
    `);
    ...
    // Step 5: Check if inventory ledger exists and is updated
    ...
    if (!isBalanced) {
      ...
      console.log('   This indicates a problem with the double-entry bookkeeping');
      console.log('   The voucher may need additional ledger entries');
    }
    ...
    if (!hasGSTEntries && gstLedgers.length > 0) {
      ...
      console.log('   This might indicate incomplete GST posting');
    }
    
    if (inventoryLedgers.length === 0) {
      console.log('\n⚠️  ISSUE: No inventory ledgers found');
      console.log('   Inventory transactions should post to inventory ledgers');
      console.log('   Consider creating Stock-in-Hand or Inventory ledgers');
    }
    ...
    if (gstLedgers.length === 0) {
      console.log('• Create GST ledgers (CGST Input, SGST Input, IGST Input)');
    }
