...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    // ============================================
    // This migration should ONLY run on tenant databases
    // NOT on finvera_master or finvera_db
    ...
    const detectDatabaseType = async () => {
      try {
        ...
        // Skip if it's master or admin database
        if (databaseName === 'finvera_master' || databaseName.includes('master')) {
          return 'master';
        }
        ...
        // Default: assume tenant if not master or admin
        return 'tenant';
      } catch (error) {
    ...
    // CRITICAL: Skip this migration if running on master or admin database
    if (dbType === 'master') {
      console.log('⚠️  SKIPPING: This is finvera_master database. Tenant migration will not run here.');
      return;
    }
    
    if (dbType === 'admin') {
      console.log('⚠️  SKIPPING: This is finvera_db (admin) database. Tenant migration will not run here.');
      return;
    }
    
    if (dbType === 'unknown') {
      console.log('⚠️  WARNING: Could not detect database type. Proceeding with caution...');
    }
    
    console.log('✅ Proceeding with tenant migration...\n');
